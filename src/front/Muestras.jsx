...
const Muestras = () => {
  ...
  const handleDownloadPDF = async (muestra) => {
    try {
      await PDFService.descargarPDFMuestra(muestra.id_muestra || muestra.id_muestrea || muestra._id);
    } catch (error) {
  ...
  const handleViewResultsPDF = async (muestra) => {
    try {
      await PDFService.generarPDFResultados(muestra.id_muestra || muestra.id_muestrea || muestra._id);
    } catch (error) {
      setSnackbarMessage("Error al generar el PDF de resultados: " + error.message);
      setSnackbarSeverity("error");
      setSnackbarOpen(true);
    }
  };

  const handleDownloadResultsPDF = async (muestra) => {
    try {
      await PDFService.descargarPDFResultados(muestra.id_muestra || muestra.id_muestrea || muestra._id);
    } catch (error) {
      setSnackbarMessage("Error al descargar el PDF de resultados: " + error.message);
      setSnackbarSeverity("error");
      setSnackbarOpen(true);
    }
  };
                ...
                <TableCell>
                  <Box sx={{ display: "flex", gap: 1 }} onClick={(e) => e.stopPropagation()}>
                    ...
                    {tipoUsuario !== "laboratorista" && (
                      <>
                        <ActionButton
                          tooltip="Ver PDF Muestra"
                          onClick={() => handleViewPDF(muestra)}
                          IconComponent={PictureAsPdfIcon}
                        />
                        <ActionButton
                          tooltip="Descargar PDF Muestra"
                        ...
                        {muestra.estado === "Finalizada" && (
                          <>
                            <ActionButton
                              ...
                              onClick={() => handleViewResultsPDF(muestra)}
                              IconComponent={PictureAsPdfIcon}
                              sx={{ color: '#39A900' }}
                            />
                            <ActionButton
                              tooltip="Descargar PDF Resultados"
                              onClick={() => handleDownloadResultsPDF(muestra)}
                              IconComponent={GetAppIcon}
                              sx={{ color: '#39A900' }}
                            />
                          </>
